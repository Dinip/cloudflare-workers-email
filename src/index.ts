...
export default {
	async fetch(
		...
		if (request.headers.get("Authorization") != env.AUTH_TOKEN) return new Response("Unauthorized", { status: 401 });
		try {
			...
			if (!res.ok) return new Response(`Error api status:${res.status}`, { status: 500 });
